...
impl FsWalker<'_> {
    pub fn new(ustore: Arc<RwLock<UnitedStore>>, root: Vec<String>, exclude_path: Vec<String>) -> FsWalker {
        // let index_writer: Arc<RwLock<UnitedStore>>
        let kv_store = KvStore::new("./cachedata/conf");
        FsWalker { ustore, kv_store, root, exclude_path }
    }
    ...
    fn need_walk(&mut self, path: String) ->bool{
        ...
        let last_gmt_opt = self.kv_store.get_str(format!("{}#{}", last_walk_gmt_key.clone(), path.clone()));
        let curr_ts = SystemTime::now().duration_since(UNIX_EPOCH).ok().unwrap().as_secs();
        let last_over_opt = self.kv_store.get_str(format!("{}#{}", last_walk_over_key.clone(), path.clone()));

        match last_gmt_opt {
            None => {
                self.kv_store.put_str(format!("{}#{}", last_walk_gmt_key.clone(), path.clone()), curr_ts.to_string());
                self.kv_store.put_str(format!("{}#{}", last_walk_over_key.clone(), path.clone()), "0".to_string());
                return true;
            }
            Some(last) => {
    ...
    fn walk_root(&mut self, path: String) {
        ...
        for x in WalkDir::new(path.clone()).into_iter()
            .filter_entry(|x| !self.exclude_path.iter().any(|y| x.path().to_str().unwrap().to_string().starts_with(y)))
            .filter_map(|v| v.ok()) {
        ...
        let last_walk_over_key = "last_walk_over".to_string();
        self.kv_store.put_str(format!("{}#{}", last_walk_over_key.clone(), path.clone()), "1".to_string());
    }
...
mod tests {
    ...
    #[test]
    fn t1() {
        let mut walker = FsWalker::new(Arc::new(RwLock::new(UnitedStore::new())), vec!["".to_string()], vec![]);
        walker.start();
    }
